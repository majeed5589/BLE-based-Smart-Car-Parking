/*
    Smart Car Parking System - Server for Slot 1
    - BLE Server: Hosts a BLE service to provide parking slot status for Slot 1
...
void setupBLE() {
  // Initialize BLE Device with a unique name for Slot 1
  BLEDevice::init("SmartParkingESP32_Slot1");
  ...
  Serial.println("BLE Server for Slot 1 is up and running!");
}
...
void loop() {
  ...
  if (currentDistance > DISTANCE_THRESHOLD && currentDistance != 0) {
    parkingStatus = "Slot 1 Empty";
  } else {
    parkingStatus = "Slot 1 Occupied";
  }
